...
swap_color_request::swap_color_request(client_request::base_class_properties props, int nof_cards) :
        client_request(props),
        _nof_cards(nof_cards)
{ }

swap_color_request* swap_color_request::from_json(const rapidjson::Value &json) {
    base_class_properties props = client_request::extract_base_class_properties(json);
    if (json.HasMember("nof_cards") ) {
        return new swap_color_request(props, json["nof_cards"].GetInt());
    } else {
        throw GomokuException("Could not find 'nof_cards' in swap_color_request");
    }
}

void swap_color_request::write_into_json(rapidjson::Value &json,
